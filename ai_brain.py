    ...
    def train_brain(self):
        ...
        try:
            ...
            if torch is not None and nn is not None and optim is not None:
                ...
                model.eval()
                with torch.no_grad():
                    preds = model(X_tensor)  # type: ignore
                    pred_classes = torch.argmax(preds, dim=1)  # type: ignore
        ...
        try:
            ...
            if torch is not None and nn is not None and isinstance(self.model, nn.Module):  # type: ignore
                ...
                with torch.no_grad():
                    ...
                    outputs = self.model(features_tensor)  # type: ignore
                    prediction = torch.argmax(outputs, dim=1).item()  # type: ignore
